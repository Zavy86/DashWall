...
class strNav{
	 ...
	 * @return boolean
	 */
	public function addItem($label,$url="#",$enabled=true,$class='',$style='',$tags='',$target="_self"){
		$item=new stdClass();
		$item->label=$label;
		$item->url=$url;
	 ...
	 * @return boolean
	 */
	public function addSubItem($label,$url,$enabled=true,$confirm='',$class='',$style='',$tags='',$target="_self"){
	 ...
	 * @return boolean
	 */
	public function addSubSeparator($class=''){
	 ...
	 * @return boolean
	 */
	public function addSubHeader($label,$class=''){
	...
	public function render($echo=false,$indentations=0){
		...
		foreach($this->items_array as $item){
			// check for active
			$active=false;
			if(/*$item->urlParsed->query_array['mod']==MODULE && */isset($item->urlParsed->query_array['scr']) && $item->urlParsed->query_array['scr']==SCRIPT){$active=true;}
			...
			if(count($item->subItems_array)){
				foreach($item->subItems_array as $subItem){
					if(/*$subItem->urlParsed->query_array['mod']==MODULE && */isset($subItem->urlParsed->query_array['scr']) && $subItem->urlParsed->query_array['scr']==SCRIPT){$active=true;}
